...
from .REP_block import REP_GCN0, REPconv_bn, REPTcn_Block
import math
...
class gcn_block(nn.Module):
    def __init__(self, A, in_channels=64, out_channels=128, residual=False, deploy=False, heads=3):
        ...
        self.conv0 = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1))
        self.space_att = REP_GCN0(heads=heads, num_point=25, deploy=deploy)
        self.conv_bn1 = REPconv_bn(in_channels, out_channels, kernel_size=1, stride=1, padding=0, deploy=deploy,
        ...
        if not residual:
            ...
            self.residual = REPconv_bn(in_channels, out_channels, kernel_size=1, stride=1, padding=0, deploy=deploy,
                                       nonlinear=None, single_init=True)
    ...
    def forward(self, x):
        out = self.conv0(x)
        out = self.space_att(out)
        out = self.conv_bn1(out) + self.residual(x)
